...
`@yume-chan/stream-extra` package re-exports the native `ReadableStream`, `WritableStream`, and `TransformStream` classes from `globalThis`, with bundled type definitions for them.
...
`WritableStream` is the opposite of `ReadableStream`, it represents a stream of data that can be written to. To create a `WritableStream`, you only need to provide a data sink:

```ts transpile
const stream: WritableStream<number> = new WritableStream<number>({
  write(chunk) {
    console.log(chunk);
  },
  close() {
...
In TypeScript, the generic parameter of `WritableStream` is the type of the data it consumes.
...
`ReadableStream`s can be piped to `WritableStream`s:
...
const writeable = new WritableStream<number>({
  write(chunk) {
    console.log(chunk);
  },
  close() {
...
Browser's native `ReadableStream`s can only `pipeTo` native `WritableStream`s, this allows the runtime to optimize the performance, but also means you can't mix streams from different packages (like polyfills).
...
const writeable = new WritableStream<string>({
  write(chunk) {
    console.log(chunk);
  },
  close() {
