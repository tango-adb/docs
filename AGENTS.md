...
This document outlines the formatting rules and guidelines for maintaining consistent documentation across the Tango ADB project.
...
## Frontmatter & Navigation
 ...
 - Include `sidebar_position` in frontmatter for proper navigation ordering (except for `index.mdx` files which are ordered via `position` in `_category_.yml`) (Example: `@tango-adb/docs/docs/scrcpy/options/vd-system-decorations.mdx` has `sidebar_position: 49`)
 - `index.mdx` files should NOT include `sidebar_position` in frontmatter as their order is determined by `position` in the parent `_category_.yml` file (Example: `@tango-adb/docs/docs/scrcpy/options/index.mdx`)
...
## Version-Specific Content
 ...
 - For version-dependent types that change entirely, use two version spans: `<Version until="VERSION">old type</Version><Version since="VERSION">new type</Version>` (Example: `@tango-adb/docs/docs/scrcpy/options/lock-video-orientation.mdx` shows the change from `-1 | 0 | 1 | 2 | 3` to `-2 | -1 | 0 | 1 | 2 | 3`)
 - For new additions that didn't exist in older versions, use a single version span: `<Version since="VERSION">new addition</Version>` (Example: `@tango-adb/docs/docs/scrcpy/options/vd-destroy-content.mdx` uses `:::info\n\nAdded in Scrcpy 3.1\n\n:::` instead)
 - For version-dependent types, instead of using inline version spans, use a sub-list format with one list item per version range: `* **Type**:`, `* <Version until="VERSION">type before</Version>`, `* <Version since="VERSION">type after</Version>` (Example: `@tango-adb/docs/docs/scrcpy/options/lock-video-orientation.mdx`)
 - For new options, add a version info box using the `:::info` syntax: `:::info\n\nAdded in Scrcpy VERSION\n\n:::` and remove the Changelog section (Example: `@tango-adb/docs/docs/scrcpy/options/vd-system-decorations.mdx`)
 - When using version spans, remember to add the import statement `import Version from "../version-span";` at the top of the MDX file (but only if you actually use the Version component in the file) (Example: `@tango-adb/docs/docs/scrcpy/options/audio-source.mdx` has the import and uses it)
 - Remove any existing version spans that appear below the type declaration, and move the information to a Changelog section (Example: `@tango-adb/docs/docs/scrcpy/options/lock-video-orientation.mdx`)
 - For list items that change based on version, apply version spans appropriately: use pairs for replacements or single spans for new additions (Example: `@tango-adb/docs/docs/scrcpy/options/audio-source.mdx` shows this with `playback` option added in v2.6)
 - When adding new options, also add them to the list in `@tango-adb/docs/docs/scrcpy/options/index.mdx` using the proper version span syntax (Example: see how `vdDestroyContent` was added to `@tango-adb/docs/docs/scrcpy/options/index.mdx`)
 - Both added and deprecated info boxes can coexist in the same documentation file, with the `:::info` box appearing first to indicate when the option was originally added, followed by the `:::danger` box indicating when it was deprecated (Example: `@tango-adb/docs/docs/scrcpy/options/lock-video-orientation.mdx` has both)
