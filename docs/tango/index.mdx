...
```mermaid
flowchart TD
    A[/an AdbTransport instance/] --> B["new Adb()"] --> C[\an Adb instance\]
...
```mermaid
flowchart TD
    A["AdbDaemonTransport.authenticate()"] --> B[/an AdbTransport instance/]
    ...
    D --> B
    E[Custom Transport] --> B

    B --> F["new Adb()"] --> G[\an Adb instance\]
