...
```mermaid
flowchart LR
    T["Custom Transport"] -- WebSocket --> S["Transport on Backend Server"] -.-> D["Device"]
```
...
```mermaid
flowchart LR
    T["Custom Transport"] -- WebSocket --> S["Backend Server"] -- WebSocket --> R["Transport on Remote PC"] -.-> D["Device"]
```
