...
If you pipe a `ReadableStream` to `stdin`, you can close `stdin` by closing the `ReadableStream`:
...
### 💥🔄 Sync API no longer needs manual socket management
...
Or with Explicit Resource Management feature:
...
{
  using socket = await adb.sync.acquireSocket();
  // Use socket directly for custom operations
}

// Socket is automatically released after the block
```

Or with the `withSocket()` helper function:
...
- Choose an appropriate storage implementation:
  - `TangoIndexedDbStorage`: Uses IndexedDB for storing multiple keys with unlimited capacity. It can read keys stored by previous versions of Tango.
