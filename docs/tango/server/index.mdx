...
```mermaid
flowchart TB
    A["new AdbServerNodeTcpConnector()"] --> B[/"an AdbServerNodeTcpConnector instance"/] --> C["new AdbServerClient()"]
    D[/Custom connector/] --> E["new AdbServerClient()"]

    subgraph F ["an AdbServerClient instance"]
        G["getDevices()"]
        M["trackDevices()"]
        H["createTransport()"]
        N["createAdb()"]
    ...
    G & M --> I[\"an AdbServerClient.Device instance"\] --> H --> J[\"an AdbTransport instance"\] --> K["new Adb()"] --> L[\"an Adb instance"\]
    I --> N --> L

    classDef link text-decoration: underline;
    class A,B,C,E,G,H,L,M,N link;

    click A "./client#nodejs"
    click B "./client#nodejs"
    click C "./client#create-client-1"
    click E "./client#create-client-1"
    click G "./devices"
    click H "./transport"
    click L "../../api"
    click M "./watch"
    click N "./transport/#create-an-adb-instance"
```
