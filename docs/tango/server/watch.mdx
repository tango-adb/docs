...
The returned generator will yield the current device list when it's changed. By using an async generator instead of a callback, it can throw errors when something goes wrong.
...
Although the generator yields the current device list, it doesn't tell which device is connected or disconnected. You need to diff the device list yourself.
