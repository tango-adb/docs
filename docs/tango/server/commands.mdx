...
All device commands uses the `AdbServerClient.DeviceSelector` type to select the target device.
...
- `{ tcp: true }`: Selects the only TCP (ADB over Wi-Fi or emulator) device. If there are multiple TCP devices, an error will be thrown. (Same as Google ADB's `-e` option)
...
If an `AdbServerClient.DeviceSelector` object has multiple fields, only one of them will be used, based on the order above.

Because `AdbServerClient.Device` has a `transportId` field, it can also be used as a `AdbServerClient.DeviceSelector`.
