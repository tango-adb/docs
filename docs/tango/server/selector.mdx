...
Because `AdbServerClient.Device` has a `transportId` field, it can also be used as a `DeviceSelector`.

It will select the device with the specified transport ID.
