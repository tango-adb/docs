...
However, this is not a good practice. If the `ReadableStream` produces data faster than downstream `WritableStream` can consume, the buffered data will take more and more memory. To know when you can enqueue data, you can use the `pull` method:
