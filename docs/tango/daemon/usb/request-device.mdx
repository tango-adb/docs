...
On Android platform, USB permission doesn't persist after the device is unplugged and replugged.
...
Due to a [Chromium bug](https://issues.chromium.org/issues/443361683), calling `.forget()` on a device, then `.requestDevice()` the same device immediately will return a non-functional device.

It's recommended to create your own ignore list of forgotten devices, instead of using `.forget()`.
