...
On Android platform, reading device's serial number requires the permission to access the device in the first place, so `serialNumber` filter will not work for unauthorized devices.

Also, USB permission on Android platform doesn't persist after the device is unplugged and replugged.
