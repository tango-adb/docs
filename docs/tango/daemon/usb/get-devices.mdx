...
To only get ADB devices, the `AdbDaemonWebUsbDeviceManager#getDevices` method also accepts a `filters` option, to only return devices that match the specified criteria.
