...
flowchart TD
    ...
    B --> M[\"an AdbDaemonWebUsbDeviceManager instance"\]
    M --> O[".trackDevices()<br/>(Watch device changes)"]
    M --> D[".getDevices()<br/>(Get existing devices)"]
    M --> C[".requestDevice()<br/>(Add a new device)"]
    C & D & O --> E[\an AdbDaemonWebUsbDevice instance\]
    E --> F[".connect()"]
    F --> G[\an AdbDaemonConnection instance\]

    subgraph Auth ["Common steps"]
        G --> H["AdbDaemonTransport.authenticate()"]
        I[/an AdbCredentialStore instance/] --> H
        G --> P["new AdbDaemonTransport()<br/>(If authentication not required)"]
        H & P --> J[\an AdbDaemonTransport instance\]
        J --> K["new Adb()"]
        K --> L[\an Adb instance\]
    end

    classDef link text-decoration: underline;
    class B,N,C,D,O,F,H,I,L link;
    ...
    click H "../connect-device"
    click I "../credential-store"
    click L "../../../api/"
```
