...
```mermaid
flowchart TB
    subgraph A["An AdbDaemonWebUsbDeviceManager instance"]
        B["trackDevices()"]
    end

    D[/"A USB instance<br/>(Runtime WebUSB API)"/] --> E["AdbDaemonWebUsbDeviceObserver.create()"]

    subgraph C["An AdbDaemonWebUsbDeviceObserver instance"]
        F[current]
        G[onListChange]
        H[onDeviceAdd]
        I[onDeviceRemove]
    ...
    B & E --> C[\"An AdbDaemonWebUsbDeviceObserver instance"\]

    F & G --> J[\"An AdbDaemonWebUsbDevice[]<br/>(For all devices)"\]
    H & I --> K[\"An AdbDaemonWebUsbDevice[]<br/>(For changed devices)"\]

    classDef link text-decoration: underline;
    class B,E,F,G,H,I link;

    click B "#from-adbdaemonwebusbdevicemanager"
    click E "#standalone-usage"
    click F "#current"
    click G "#onlistchange"
    click H "#ondeviceadd-and-ondeviceremove"
    click I "#ondeviceadd-and-ondeviceremove"
```
