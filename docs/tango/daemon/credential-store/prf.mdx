...
The most basic usage is with a [LocalStorage storage](./index.mdx#tangolocalstorage) and a WebAuthn PRF source.
...
Or, an [IndexedDB storage](./index.mdx#tangoindexeddbstorage) can also be used. Note that since `TangoWebAuthnPrfSource` can't run in Web Workers, so using it doesn't have any big benefits.
