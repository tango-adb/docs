...
The most common usage is to use `AdbDaemonAuthenticator.authenticate()` static method to create an authenticated transport instance. It requires an [`AdbDaemonConnection`](#connections) object to send and receive packets and an [`AdbCredentialManager`](./credential-store/index.mdx) object to provide authentication keys.

```mermaid
flowchart TD
    A[/an AdbDaemonConnection<br>object/] & B[/an AdbCredentialManager<br/>object/] --> C["AdbDaemonAuthenticator<br/>.authenticate()"] --> D[\an AdbDaemonTransport<br/>instance\] --> E["new Adb()"] --> F[\an Adb instance\]
...
flowchart TD
    ...
    D & E[/an AdbCredentialManager<br/>object/] --> F["AdbDaemonAuthenticator<br/>.authenticate()"] --> G[\an AdbDaemonTransport<br/>instance\] --> H["new Adb()"] --> I[\an Adb instance\]
