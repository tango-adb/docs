...
- [`subprocess`](./adb/subprocess/index.mdx): Spawn processes on device
...
- [`createTransport`](./server/create-transport.mdx): Create an `AdbTransport` object for the device, for low-level usages
- [`createAdb`](./server/create-adb.mdx): Creates an `Adb` object for the device, for high-level usages
...
- [`pm`](./bin/pm/index.mdx): Manage apps
  - [`listPackages`](./bin/pm/list-packages.mdx): Query installed packages with filtering options
  - [`getPackageSources`](./bin/pm/get-package-sources.mdx): Get APK file paths for a package
...
- [`am`](./bin/am/index.mdx): Manage activities and broadcasts
  - [`startActivity`](./bin/am/start-activity.mdx): Start Android activities with specific intents
  - [`broadcast`](./bin/am/broadcast.mdx): Send broadcast intents to registered receivers
  - [`Intent Format`](./bin/am/intent-format.mdx): Details about the Intent interface and its properties
