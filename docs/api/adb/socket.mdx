...
In Tango, a socket is represented by an `Adb.Socket` object, while each transport may extend it with additional properties.
...
```ts
declare class Adb {
  createSocket(address: string): Promise<Adb.Socket>;
}
```
