...
:::danger[Both `stdout` and `stderr` must be used!]

Despite through multiple layers of redirection, `stdout` and `stderr` are precise emulations of the process's `stdout` and `stderr` streams. If data in the streams is not consumed, after the internal buffer is full, the process will (very likely) hang when trying to write more data to `stdout` or `stderr`.

Shell protocol transmits `stdout` and `stderr` in one ADB socket, so not reading from either stream will block the whole socket and cause the above issue.

You must continuously read both streams (either by piping them to `WritableStream`s, using `for await` loop, or calling `reader.read()` in a loop) to prevent this from happening.

If the remaining data of a stream is not needed, `stream.cancel()` (or `reader.cancel()` if using a reader) can be called to discard them.
...
- If the process exits normally using `exit(code)`, the promise resolves with `code & 0x7f`.
- If the process is killed by a signal, the promise resolves with `128 + signal number`. (For example, `SIGKILL` is signal number `9`, so the exit code will be `128 + 9 = 137`)
- If the [`signal`](#signal) option aborts the process, the promise rejects with `signal.reason`.
- If the process is killed by calling [`kill()`](#kill), the promise rejects with a "Socket ended without exit message" error.
...
:::info[Important: Consumption Once]
The result of `spawn()` can only be consumed once. You must choose one of these approaches:

1. Await the promise directly to get the process object for manual interaction
2. Use the wait interface to get the output directly when the process completes

You cannot use both approaches with the same result of `spawn()`.
:::
...
```ts showLineNumbers
export declare class AdbShellProtocolPty {
...
declare const pty: AdbShellProtocolPty;
