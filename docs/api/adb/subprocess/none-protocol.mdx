...
- If the process exits normally, was killed by a signal, or was killed by calling [`kill()`](#kill), the promise resolves with `undefined`.
...
```ts showLineNumbers
export declare class AdbNoneProtocolPty {
...
export declare class AdbNoneProtocolSubprocessService {
  pty(command?: string | string[]): Promise<AdbNoneProtocolPty>;
}
```
