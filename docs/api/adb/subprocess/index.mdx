...
| `AdbNoneProtocolPty`  | None     | PTY  | ⛔      | ✅      | ⛔       | ⛔       | ✅       | ⛔       | ✅       | `Promise<undefined>` |
| `AdbShellProtocolPty` | Shell    | PTY  | ⛔      | ✅      | ⛔       | ⛔       | ✅       | ✅       | ✅       | `Promise<number>`    |
...
:::danger[Outputs must be used!]

Despite through multiple layers of redirection, `output` (or `stdout` + `stderr`) is a precise emulation of the process's `stdout` and `stderr` streams. If data in the streams is not consumed, after the internal buffer is full, the process will (very likely) hang when trying to write more data to `stdout` or `stderr`.
...
You must continuously read all streams (either by piping them to `WritableStream`s, using `for await` loop, or calling `reader.read()` in a loop) to prevent this from happening.
