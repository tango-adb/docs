...
Each socket also has a service name, which is used to identify the service that the socket is connected to. It's similar to the hostname and port in TCP sockets.
...
While the service name usually points to a service on the ADB daemon, it can also be a native socket address on the device.

In Google ADB, forward tunnels must be associated with a local address, and the client needs connect to that address to have the connection forwarded to the device. In Tango, the client can connect to a device address directly.
