...
Each socket also has a service name, which  similar to the hostname and port number in a TCP socket, or the URL in an HTTP request
...
In Google ADB, forward tunnels must be associated with a local address, and the client needs connect to that address to have the connection forwarded to the device.
...
In Tango, only the last step is needed. The `createSocket` method can be used to connect to the device address directly.
...
If your target runtime environment supports listening on a TCP port, you can simulate the behavior of `adb forward` by listening on a TCP port and forwarding the connection to `createSocket`.

### Server Transport

Support for `adb forward` series of commands in [Server Transport](../tango/server/index.mdx) has not been implemented yet.

### Address family

Supported address families includes:
