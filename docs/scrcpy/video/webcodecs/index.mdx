...
The WebCodecs decoder is more powerful and flexible than H264BSD decoder, but also slightly more difficult to use. Here's a flowchart showing the overall steps to create and use WebCodecs decoders:
...
If WebCodecs API is not supported, you can fallback to the [H264BSD decoder](../tiny-h264.mdx), but be aware only H.264 Baseline profile level 4 is supported, and the performance is much worse.
...
After checking browser and codec support, create a `WebCodecsVideoDecoder` instance. If no `renderer` option is specified, it automatically creates an `AutoCanvasRenderer` that selects the best available renderer: hardware-accelerated `WebGLVideoFrameRenderer` when available, falling back to software-based `BitmapVideoFrameRenderer` otherwise. See [Renderers](./renderer.mdx) for advanced renderer configuration.

```ts transpile
import { WebCodecsVideoDecoder } from "@yume-chan/scrcpy-decoder-webcodecs";
import type { ScrcpyVideoCodecId } from "@yume-chan/scrcpy";

declare const codec: ScrcpyVideoCodecId;

const decoder = new WebCodecsVideoDecoder({ codec });
```

The `renderer` property returns the `AutoCanvasRenderer` instance, which owns a `<canvas>` element for rendering. Append it to the DOM to display the video:

```ts transpile
document.body.appendChild(decoder.renderer.canvas);
```

When the renderer switches between WebGL and Bitmap (e.g., after WebGL context loss), the `canvas` property may change to a new element. Listen to the `onRendererTypeChange` event to handle this:

```ts transpile
decoder.onRendererTypeChange?.((type) => {
    console.log(`Renderer type changed to: ${type}`);
    // Re-append the (possibly new) canvas to the DOM
    document.body.appendChild(decoder.renderer.canvas);
});
```

For more in-depth usage, see:

- [Renderers](./renderer.mdx) - All renderer types, the `onTypeChanged` event, and the `createCanvas` option
- [Decoder operations](./decoder.mdx) - Piping video streams, handling size changes, and managing decoder lifecycle
