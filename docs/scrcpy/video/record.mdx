...
The [`MediaRecorder`](https://developer.mozilla.org/en-US/docs/Web/API/MediaRecorder) API can record the rendering canvas (from [TinyH264 decoder](./tiny-h264.mdx) or [WebCodecs decoder](./webcodecs/index.mdx#create-a-renderer)) into a MP4 or WEBM file.

Note that for [`WebGLVideoFrameRenderer`](./webcodecs/index.mdx#create-a-renderer), the `enableCapture` parameter must be set to `true`.
...
```sh npm2yarn
npm install @yume-chan/media-codec @yume-chan/scrcpy @yume-chan/stream-extra mediabunny
```
    ...
    async write(chunk) {
      ...
      if (videoCodec === ScrcpyVideoCodecId.AV1) {
        ...
        if (sequenceHeader) {
          ...
          const codecString = Av1.toCodecString(sequenceHeader);
      ...
      if (videoCodecConfiguration) {
        ...
        if (videoCodec === ScrcpyVideoCodecId.H264) {
          const configuration = H264.parseConfiguration(
            videoCodecConfiguration
          );
          codedWidth = configuration.encodedWidth;
          codedHeight = configuration.encodedHeight;
          codecString = H264.toCodecString(configuration);
        } else {
          const configuration = H265.parseConfiguration(
            videoCodecConfiguration
          );
          codedWidth = configuration.encodedWidth;
          codedHeight = configuration.encodedHeight;
          codecString = H265.toCodecString(configuration);
        }
