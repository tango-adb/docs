...
Microsoft Edge on Windows uses Media Foundation decoders.

Decoding H.265 requires the [HEVC Video Extensions](https://www.microsoft.com/store/productId/9NMZLZ57R3T7) or HEVC Video Extensions from Device Manufacturer app from Microsoft Store. But it has been reported that the H.265 decoder outputs incorrect sized frames.

Decoding AV1 requires the [AV1 Video Extension](https://www.microsoft.com/store/productId/9MVZQVXJBQ9V) app from Microsoft Store.
