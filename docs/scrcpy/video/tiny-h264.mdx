...
It wraps [`tinyh264`](https://npmjs.org/package/tinyh264) package, which compiles the C code into WebAssembly, and runs it in a Web Worker. This way, the main thread is not blocked by the decoding process.
...
Tiny H264 decoder outputs raw YUV frames, which needs to be converted to RGB before rendering.

It uses [`yuv-canvas`](https://www.npmjs.com/package/yuv-canvas) package to do the conversion. When supported, it uses a WebGL shader to accelerate the conversion, so it's very fast. But on unsupported devices, it falls back to a software implementation, which is super slow.
