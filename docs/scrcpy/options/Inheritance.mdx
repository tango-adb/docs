...
`ScrcpyOptionsX_XX` classes use a special inheritance mechanism to allow breaking changes between versions. When features change in the server, a new options class is created, and only the changed behaviors are overridden.
