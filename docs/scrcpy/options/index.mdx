...
- `spawner`: A custom [process spawner](../../api/adb/subprocess.mdx) to start the server process. The default value is `undefined`, which uses `adb.subprocess.noneProtocol`. This option is rarely used, unless you have a different method (like SSH) to start processes on the device.
