...
- [`videoBitRate`](./video-bit-rate.mdx): Sets the video bitrate for the stream in bits per second
- [`videoCodecOptions`](./video-codec-options.mdx): Configures video encoding parameters for the selected video codec
- [`videoEncoder`](./video-encoder.mdx): Specifies the name of the encoder to use for video encoding
