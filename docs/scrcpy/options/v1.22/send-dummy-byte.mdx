...
By default, sending a dummy byte is enabled in Scrcpy when using forward tunnel mode (controlled by the [`tunnelForward`](../v1.15/tunnel-forward.mdx) option), meaning the server sends a `0` byte at the start of the video stream to detect connection issues.
