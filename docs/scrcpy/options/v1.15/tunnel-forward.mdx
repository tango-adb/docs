...
This option determines how the Scrcpy client and server communicate through ADB. By default, Scrcpy uses reverse tunnels where the client listens on the PC and the server connects to it. When `tunnelForward` is set to `true`, the server listens on the device and the client connects to it from the PC.
