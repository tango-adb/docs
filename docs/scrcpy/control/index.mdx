...
- [Text input](./keyboard-text.mdx): Inject text directly into the device.
...
- [Panel controls](./panel.mdx): Expand or collapse notification and settings panels.
- [Rotate device](./rotate.mdx): Change the device orientation.
- [Set clipboard](./clipboard.mdx): Set the device clipboard content.
- [USB HID devices](./uhid.mdx): Control USB Human Interface Devices (HID) on the device.
- <Version since="v3.0">[Reset video](./reset-video.mdx): Restart video capture and encoding.</Version>
- [Start app](./start-app.mdx): Start applications on the device by package name.
