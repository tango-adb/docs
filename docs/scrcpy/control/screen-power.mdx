...
If [`cleanup: true`](../options/cleanup.mdx) (the default value) is set in the `ScrcpyOptions`, Scrcpy server will restore the screen state to `Normal` on exit. Pressing the physical power button twice usually turns the screen back on.
