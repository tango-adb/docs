...
The `scid` (session identifier) option allows multiple Scrcpy sessions to run simultaneously by providing each session with a unique socket name. 

This is particularly useful when using forward tunnels, as each session needs to bind to a different address to avoid conflicts.
...
When using ADB forward tunnels (controlled by the [`tunnelForward`](./options/v1.15/tunnel-forward.mdx) option), ADB server listens on the PC. 
When client connects to the local socket address, it will always succeed.

However, this connection success doesn't guarantee that the Scrcpy server on the Android device has successfully established its end of the connection.

To signal that the server-to-client connection is truly established, the server sends an extra `0x00` byte at the beginning of the first established [socket](#sockets) (usually, the video socket). 

This mechanism is controlled by the [`sendDummyByte`](./options/v1.22/send-dummy-byte.mdx) option, which is enabled by default in forward tunnel mode.

You need to ignore this dummy byte before sending the stream to other APIs to process. 

The dummy byte is only sent on the first socket (video, audio, or control, depending on which is enabled first). Subsequent sockets in the same session will not contain a dummy byte.
...
This behavior can be turned off using the [`sendDummyByte: false`](./options/v1.22/send-dummy-byte.mdx) option. For example, in [daemon transport](../tango/daemon/index.mdx) mode, it connects to ADB daemon directly, so no dummy byte is needed to detect connection success. Additionally, when using reverse tunnels (the default mode), the connection establishment is handled by ADB's reverse tunnel mechanism, so dummy bytes are not needed.
...
However, if [`reverse.add`](../api/adb/reverse/add.mdx) throws an [`AdbReverseNotSupportedError`](../api/adb/reverse/index.mdx#errors), `AdbScrcpyClient` will automatically modify the options to use forward tunnel instead, which also enables the `sendDummyByte` option by default to handle connection detection.
