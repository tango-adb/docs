...
`videoStream` field contains the `AdbScrcpyVideoStream` object<Version since="v2.1">, or `undefined` if [`video: false`](./options/v2.1/video.mdx) option is specified.</Version>
...
Controller is used to send [control messages](./control/index.mdx) to server<Version since="v2.1">, or `undefined` if [`control: false`](./options/v1.15/control.mdx) option is specified.</Version>
...
If [`control: false`](./options/v1.15/control.mdx) option is not specified<Version since="v2.1"> and [`clipboardAutosync: false`](./options/v1.21/clipboard-autosync.mdx) option is also not specified</Version>, Scrcpy server will monitor device clipboard and send the change to client.
